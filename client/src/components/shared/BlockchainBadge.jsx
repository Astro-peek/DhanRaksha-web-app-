...
export default function BlockchainBadge({ txHash, className = '' }) {
  ...
  return (
    <a
      ...
      className={cn(
        "group inline-flex max-w-full items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-primary/10 text-primary border border-primary/30 hover:bg-primary hover:text-white transition-all duration-200 shadow-sm hover:shadow-md whitespace-nowrap",
        className
      )}
      title={`Transaction: ${txHash}`}
    >
      <Link2 size={12} className="animate-pulse group-hover:rotate-45 transition-transform duration-300" />
      <span className="whitespace-nowrap">On Polygon</span>
      <span className="hidden sm:inline font-mono text-[10px] ml-0.5 opacity-80 group-hover:opacity-95">
        ({truncatedHash})
      </span>
